...
// In-memory storage for bookings (for development without database)
...
const updateBooking = async (bookingId, updates) => {
  // Find booking by bookingId or paymentId
  let booking = null;
  for (const [key, value] of bookings.entries()) {
    if (value.bookingId === bookingId || value.paymentId === bookingId) {
      booking = value;
      break;
    }
  }
  ...
  bookings.set(booking.bookingId, updatedBooking);
...
const startServer = async () => {
  try {
    // Skip database initialization for in-memory storage
